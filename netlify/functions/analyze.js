...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Route fetch_psi : proxy PageSpeed Insights avec cle API cachee
    if (body.action === 'fetch_psi') {
      const targetUrl = body.url;
      if (!targetUrl) return { statusCode: 400, headers: { 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify({ error: 'url required' }) };

      const psiKey = process.env.PSI_API_KEY;
      let api = 'https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=' + encodeURIComponent(targetUrl) + '&strategy=mobile&category=performance&category=seo&category=accessibility&category=best-practices';
      if (psiKey) api += '&key=' + psiKey;

      try {
        const res = await fetch(api, { signal: AbortSignal.timeout(15000) });
        if (!res.ok) {
          return { statusCode: 200, headers: { 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify({ error: 'psi_unavailable', status: res.status }) };
        }
        const data = await res.json();
        const cats = (data.lighthouseResult && data.lighthouseResult.categories) || {};
        const au = (data.lighthouseResult && data.lighthouseResult.audits) || {};
        const result = {
          ...
          fcp: (au['first-contentful-paint'] && au['first-contentful-paint'].displayValue) || null,
          lcp: (au['largest-contentful-paint'] && au['largest-contentful-paint'].displayValue) || null,
          cls: (au['cumulative-layout-shift'] && au['cumulative-layout-shift'].displayValue) || null,
          hasViewport: (au['viewport'] && au['viewport'].score) === 1,
          hasMetaDesc: (au['meta-description'] && au['meta-description'].score) === 1,
          hasDocTitle: (au['document-title'] && au['document-title'].score) === 1,
          pageTitle: (au['document-title'] && au['document-title'].details && au['document-title'].details.items && au['document-title'].details.items[0] && au['document-title'].details.items[0].snippet) || null
        };
        return { statusCode: 200, headers: { 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify(result) };
      } catch (e) {
        return { statusCode: 200, headers: { 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify({ error: 'psi_fetch_failed', message: e.message }) };
      }
    }
